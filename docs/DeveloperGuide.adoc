...
The mechanism is facilitated by a new command `FindAlphabets` will search for contacts matching the sequence of alphabets in the `commandbox`. The command is called every time they user enters or remove a character from the command box while using the command `Find`.
...
**Cons:** Removing the find Command will affect other functions of the Addressbook such as the `findtags` function which works better with the original `FindCommand`.
