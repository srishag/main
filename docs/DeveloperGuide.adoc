...
The key release event of the command box is constantly searching for the term `find` and when the user inputs the mentioned term into the command box, the key release event will begin searching for contacts by passing the letters entered after the term `find` into `FindAlphabetsCommand`.
...
**Cons:** Removing the find Command may affect other functions of the address book.
// end::EasyFind[]
