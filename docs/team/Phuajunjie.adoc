...
The main theme of Contact'em is to manage and store contacts.  As the use of Smart phones is becoming more prevalent in the world right now, being able to access contacts on the go has became the priority of many users.
...
The easyfind feature improves on the find feature. Contact'em now auto updates the display whenever user inputs a letter with the `find` command word.
...
image::EasyFind example.png[width="200"]
...
The mechanism is facilitated by a new command `FindLettersCommand` which will search for contacts matching the sequence of letters in the command box. The command is called every time the user inputs or remove a character from the command box while using the command `Find`.
...
The application will display the number of contacts that share the same letter sequence as the input. The following images shows how the EasyFind mechanism is activated:
