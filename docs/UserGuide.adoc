...
=== Adding a Person: `add`
`Add` is the most basic functionality of Contact'em, which adds a contact to the address book.
...
* `add n/Betsy Crowe t/friend e/betsycrowe@example.com a/Betsy road, block 456, #01-02 p/1234567 t/friend`

If contact was added in error, or with errors, user can `undo`, `delete`, or `edit` it. +
To undo, type `undo` (recommended). +
To edit, refer to section xxx below (recommended for advanced users). +
To delete, refer to section xxx below (least recommended). +

[TIP]
Parameters in square brackets above, such as Birthday, Facebook and Tag, are optional when first adding a contact
...
[WARNING]
Prefixes are necessary before the corressponding fields, e.g. **n/** John will work, but just entering John will not.
...
[WARNING]
If this occured due to an accidental `clear` command, it is recommended to `undo` as soon as possible, #without closing the application as data will be permanently lost after otherwise.#
