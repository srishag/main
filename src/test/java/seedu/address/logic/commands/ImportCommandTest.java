...
public class ImportCommandTest {
    ...
    public void execute_assert_CommandSuccess() throws DuplicatePersonException,CommandException{
        ...
        ImportCommand command = prepareCommand(personList, this.model);
    ...
    public void execute_assert_CommandFailure_ContactInvalidFormat() throws DuplicatePersonException,CommandException{
        ...
        assert_Command_Failure(command, expectedMessage, modelstub, this.model);
    }
    ...
    public void execute_assert_CommandFailure_ContactExists() throws DuplicatePersonException,CommandException{
        Model SimilarContactModel = model;
        SimilarContactModel.addPerson(GoogleContactList.FreddyAddressBook);

        String expectedMessage = "0 contact/s imported!     1 contact/s failed to import!" + "\n" +
                "Contacts already existed : 1     Contacts not in the correct format : 0" + "\n";
        personList.add(GoogleContactList.FreddyGoogle);
        ImportCommand command = prepareCommand(personList, SimilarContactModel);

        assert_Command_Failure(command, expectedMessage, SimilarContactModel,this.model);
    }
     ...
     */
    private void assert_Command_Failure(Command command, String expectedMessage,Model modelstub, Model model) throws CommandException{
        CommandResult commandResult = command.execute();
        assertEquals(expectedMessage, commandResult.feedbackToUser);
        assertEquals(modelstub.getAddressBook(), model.getAddressBook());
    }
